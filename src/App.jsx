...
const MEALS_CONFIG = {
  ...
  editSubLabel: 'Edit title, date, and options',
  noun: 'Option',
  itemNoun: 'Option',
  pageNoun: 'Meal',
...
function AppContent() {
  ...
  // --sab is measured and cached in main.jsx (setSAB IIFE) before React mounts,
  // so it's already set correctly by the time this component renders.
  // ChatView has its own measurement before it sets body.overflow=hidden.
  ...
  useEffect(() => {
    function onKeyDown(e) {
      ...
      if (e.key === 'Escape' && !inInput) {
        ...
        else if (giving.open) giving.close()
        else if (birthday.open) birthday.close()
        return
      }
      if (inInput || e.metaKey || e.ctrlKey || e.altKey) return
      const num = parseInt(e.key)
      if (!isNaN(num) && num >= 1 && num <= visibleTabs.length) {
        handleTabChange(visibleTabs[num - 1].path)
      }
    }
  ...
  return (
    ...
    <div className="bg-sunrise-50 lg:pl-56" style={{ minHeight: 'calc(var(--dvh) + var(--sat))' }}>
      ...
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex lg:flex-col fixed inset-y-0 left-0 w-56 bg-stone-50 border-r border-stone-200 z-40">
        <div className="px-4 py-5 border-b border-stone-200 flex items-center gap-2.5">
      ...
      {!chatViewOpen && (
        <LayoutGroup id="floating-pill">
          <nav
            className={`pill-nav fixed left-1/2 -translate-x-1/2 z-40 lg:hidden bg-white/90 backdrop-blur-sm shadow-lg border border-stone-200 flex items-center py-1.5 ${visibleTabs.length <= 3 ? 'rounded-3xl px-3 gap-2' : 'rounded-full px-2'}`}
