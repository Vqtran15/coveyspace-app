...
export default function ConversationList({ session, groupId, members, enterClass, autoOpenGroupChat, onAutoOpenConsumed, autoOpenMainChat, onAutoOpenMainChatConsumed, onSelect, onRead, onOpenSettings, upcoming = [], birthdayBannerDismissed = false, birthdayBannerClosing = false, onDismissBirthdayBanner, onOpenBirthdays, pushSupported, pushSubscribed, pushPermission, pushToggling, onPushToggle, pinnedGroupId, onPinGroup }) {
  ...
  function convName(conv) {
    ...
    // Custom name always wins for group chats
  ...
  return (
    <div
      ...
      style={{ height: 'calc(100svh - env(safe-area-inset-bottom) - 62px)', paddingTop: 'env(safe-area-inset-top)' }}
    >
            ...
            {newChatMode === 'dm' && (
              <div className="px-4 pb-8 space-y-1 max-h-72 overflow-y-auto">
                {otherMembers.length === 0 ? (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full overflow-hidden flex items-center justify-center shrink-0 ${m.avatar_image_url ? 'bg-stone-200 shadow-md' : avatarColor(m.user_id, m.avatar_color)}`}>
                      {m.avatar_image_url
