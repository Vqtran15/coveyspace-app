...
export default function WelcomeSplash({ onDone }) {
  ...
  async function shareCode() {
    if (!inviteCode) return
    const url = `${window.location.origin}/login?code=${inviteCode}`
    if (navigator.share) await navigator.share({ title: 'Join my group on Coveyspace', url }).catch(() => {})
    else copyLink()
  }
      ...
      return (
        <div className="flex-1 overflow-y-auto overscroll-contain">
          <div className="w-full max-w-xs mx-auto px-6 pb-10" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
            ...
            <div className="bg-white border border-stone-100 rounded-2xl p-4 shadow mb-3 animate-fade-up" style={{ animationDelay: '0.2s' }}>
              <AvatarPicker
                userId={userId}
    ...
    if (step === 'features') return (
      <div className="flex-1 overflow-y-auto overscroll-contain">
        <div className="w-full max-w-xs mx-auto px-6 pb-10" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
          ...
          <div className="bg-white border border-stone-100 rounded-2xl overflow-hidden shadow mb-6 animate-fade-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="w-full max-w-xs mx-auto px-6 pb-10" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
            ...
            {showMeals && (
              <div className="mb-5 animate-fade-up" style={{ animationDelay: '0.15s' }}>
                ...
                <div className="bg-white border border-stone-100 rounded-2xl p-4 shadow space-y-4">
                  <div>
            ...
            {!showMeals && !showServices && (
              <div className="bg-white border border-stone-100 rounded-2xl p-6 shadow text-center mb-6 animate-fade-up" style={{ animationDelay: '0.15s' }}>
            ...
            >
              {visibleTourCards.map(({ Icon, color, title, desc }) => (
                <div key={title} className="w-full shrink-0 mr-6">
                  <div className="bg-white border border-stone-100 rounded-2xl p-8 shadow h-full flex flex-col items-center justify-center text-center gap-4">
