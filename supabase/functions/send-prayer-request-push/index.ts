...
Deno.serve(async (req) => {
  try {
    const { groupId, memberUserId, memberName, submittedByUserId } = await req.json()

    if (!groupId || !memberUserId || !memberName || !submittedByUserId) {
      return new Response('Missing fields', { status: 400 })
    }

    const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY)

    // Notify all group members except the person who submitted the form
    const { data: subs, error } = await supabase
      .from('push_subscriptions')
      .select('id, endpoint, subscription')
      .eq('community_group_id', groupId)
      .neq('user_id', submittedByUserId)
    ...
    const notification = JSON.stringify({
      ...
      body: `${memberName} shared a new prayer request`,
      url: '/prayer',
    })
